...
class Custom_WebSocket
{
	...
	/*
	 $data = {from: uniqueName, to: uniqueName, message: uniqueName} 
	 */
	...
	public $uniqueNameSocketSearch = array(); // $uniqueNameSocketSearch = array[uniqueName] = array('clientIDs'); // clientIDs are associated with sockets.
	
	/*
	 	$this->chatRooms[fromUniqueName][toUniqueName] = array(); // contains the number of instantces the destination have. 
	 * */
	...
	/*
		$this->wsClients[ integer ClientID ] = array(
			...
			12 => string   UniqueName						  // the front end display name
		)
	...
	*/
	...
	function wsStartServer($host, $port) {
		...
		$this->logger->info('web start server here');
		if (isset($this->wsRead[0])) return false;

		if (!$this->wsRead[0] = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)) { //$this->wsRead[0] is the socket resource
			return false;
		}
		if (!socket_set_option($this->wsRead[0], SOL_SOCKET, SO_REUSEADDR, 1)) { //sets the options for the socket resource, 	
			socket_close($this->wsRead[0]);
			return false;
		}
		if (!socket_bind($this->wsRead[0], $host, $port)) { //bind the socket resource to an address and port.
			socket_close($this->wsRead[0]); // if not, then close the socket resource.
			return false;
		}
		if (!socket_listen($this->wsRead[0], 10)) { // after the socket has been set up and bound to a name, then start listening to the port
			socket_close($this->wsRead[0]); // if not, then close the socket.
			return false;
		}
		...
		while (isset($this->wsRead[0])) { // while the server is instantiated. [0] is always the server.

			//Zend_Debug::dump($this->logger);
			$this->logger->info('here at 134');
			$changed = $this->wsRead; // copy wsRead into changed.
			
			//socket_select accepts arrays of sockets and waits for them to change status. 
			$result = socket_select($changed, $write, $except, 2); //number of socket resources contained in the modified arrays, which may be zero if the timeout expires before anything interesting happens.
			

			if ($result === false) { // if sockets can not be selected
				socket_close($this->wsRead[0]); // then close the socket
				return false;
			}
			elseif ($result > 0) { // if there is more than one socket resource
				foreach ($changed as $clientID => $socket) { // iterate through each socket resource
					if ($clientID != 0) { 
						$this->logger->info('clientID is ==================================> '.$clientID);
						
						// client socket changed
						$buffer = ''; // incoming data will be set to buffer first
						$bytes = @socket_recv($socket, $buffer, 4096, 0); // returns the number of bytes received
						...
						$this->logger->info('here at 170, $clientID == 0');
						...
						if ($client !== false) {
							...
							if ($result !== false && $this->wsClientCount < self::WS_MAX_CLIENTS && (!isset($this->wsClientIPCount[$clientIP]) || $this->wsClientIPCount[$clientIP] < self::WS_MAX_CLIENTS_PER_IP)) {
								$this->logger->info('here at 182, add client with $client and $clientIP');
	...
	function wsProcessClient($clientID, &$buffer, $bufferLength) {
		if ($this->wsClients[$clientID][2] == self::WS_READY_STATE_OPEN) {
			// handshake completed
			$this->logger->info('here at building frame');
		...
		elseif ($this->wsClients[$clientID][2] == self::WS_READY_STATE_CONNECTING) {
			...
			if ($result) {
				...
				if(!isset($this->wsClients[$clientID][12])){ // if it is open and no unique name set
					if ( array_key_exists('connecting', $this->wsOnEvents) ){ // then it is still trying to connect althou information is being passed.
						foreach ( $this->wsOnEvents['connecting'] as $func )
						//$this->logger->info($func);
						$this->logger->info('trying to set up uniquename');
						call_user_func_array($func, array($clientID, $this));
						//$func($clientID);
					}
				}
				/*
	...
	/**
	 * Build information from incoming frames recursivesly until all buffer frames are read.
	 * @param unknown_type $clientID
	 * @param unknown_type $buffer
	 * @param unknown_type $bufferLength
	 */
	function wsBuildClientFrame($clientID, &$buffer, $bufferLength) {
		...
		$this->wsClients[$clientID][8] += $bufferLength; //8 => integer FrameBytesRead,  // amount of bytes read for a frame, reset to 0 when all frame data has been read
		
		$this->wsClients[$clientID][9] .= $buffer; //9 => string FrameBuffer,  // joined onto end as a frame's data comes in, reset to blank string when all frame data has been read

		// check if the length of the frame's payload data has been fetched, if not then attempt to fetch it from the frame buffer
		if ($this->wsClients[$clientID][7] !== false || $this->wsCheckSizeClientFrame($clientID) == true) { //7 => int/false FramePayloadDataLength, 
																											// length of a frame's payload data, reset to false when all frame data has been read (cannot reset to 0, to allow reading of mask key)
	...
	/**
	 * Checks the size the a client frame
	 * @param unknown_type $clientID
	 */
	...
	function wsAddClient($socket, $clientIP) {
		...
		$clientID = $this->wsGetNextClientID(); // gets the next ClientID();
		...
		//$this->logger->info(Zend_Debug::dump($this->wsRead[$clientID]));
	}
	function wsRemoveClient($clientID) {
		...
		if ( array_key_exists('close', $this->wsOnEvents) )
			foreach ( $this->wsOnEvents['close'] as $func )
				call_user_func_array($func, array($clientID, $closeStatus, $this));
	...
	function wsGetNextClientID() {
		...
		while (isset($this->wsRead[$i])) $i++; // when there is not an available socket, return that id to be assigned to the next one.
		return $i;
	}
	function wsGetClientSocket($clientID) {
		return $this->wsClients[$clientID][0]; // return the socket of a clientID
	}
	...
	function wsProcessClientFrame($clientID) {
		...
		// decode payload data
		$this->logger->info('buffer data on line 539 is: '.$buffer);
		...
		$this->logger->info('decoded data on line 553 is: '.$data);
		
		$decodedData = json_decode($data, true); // decode data;
		if ( array_key_exists($decodedData['type'], $this->wsOnEvents) ){
			$this->logger->info('data type '.$decodedData['type'].'is available on line 557');
				foreach ( $this->wsOnEvents[$decodedData['type']] as $func )
					call_user_func_array($func, array($clientID, $decodedData, $this));
					//$func($clientID, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY);
		}else{ // else process the message forward.
	...
	function wsProcessClientMessage($clientID, $opcode, &$data, $dataLength) {
		// check opcodes
		$this->logger->info('data at 625 : '.$data);
		...
		elseif ($opcode == self::WS_OPCODE_TEXT || $opcode == self::WS_OPCODE_BINARY) {
			if ( array_key_exists('message', $this->wsOnEvents) )
				foreach ( $this->wsOnEvents['message'] as $func )
					call_user_func_array($func, array($clientID, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY, $this));
					//$func($clientID, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY);
		}
		else {
			// unknown opcode
	...
	function wsProcessClientHandshake($clientID, &$buffer) {
		...
		do {
			...
			$this->logger->info('process client handshake at line 689. headers are the following...');
