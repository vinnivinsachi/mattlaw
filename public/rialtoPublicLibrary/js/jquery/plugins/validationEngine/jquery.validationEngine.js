    ...
    var methods = {
        ...
        _buildPrompt: function(field, promptText, type, ajaxed, options) {
            ...
            if (options.isOverflown)
              ...
              // $("body").append(prompt);
				// add the element as a sibling of the form input
        ...
        _calculatePosition: function(field, promptElmt, options) {
			...
			var fieldMarginTop = parseInt(field.css('marginTop'));
            var promptHeight = promptElmt.height();
			var promptWidth = promptElmt.width();
			var fieldParent = field.parent();
			
			// get surrounding document for testing if error message is off screen
				var surroundingDocument = $(window);
				if($('#fancybox-content').has(field).length) { surroundingDocument = $('#fancybox-content'); }

			// check if off the screen and switch sides if needed
				if((fieldParent.offset().left + field.position().left + field.width() + promptWidth) > surroundingDocument.width()) {
					promptElmt.children('.formErrorContent').addClass('version-left');
				}
            ...
            switch (options.promptPosition) {
                ...
                case "topRight":
                    if (overflow)
                        ...
                        promptleftPosition += fieldWidth - 30;
                        promptTopPosition += -promptHeight + fieldMarginTop;
                    }
                    break;
                case "topLeft":
                    promptTopPosition += -promptHeight - 10 + fieldMarginTop;
                    break;
                case "centerRight":
