...
var FancySelect = new Class({
	...
	select: function(value) {
		//addjust price based on hidden select value
		
		this.element.set('value', value); // Update hidden <select> // ah. setting values on hidden select dosn't ahve to add attributes to the options underneath.
		var beforeModifier = this.element.get('modifier');
		var newModifier = this.selectOptions[value].modifier;
		var difference = newModifier - beforeModifier;
		
		this.element.set('modifier', this.selectOptions[value].modifier);
		this.updateModifier(difference);
		...
		if (this.ul) {
			this.ul.getElements('li').each(function(li) {
				if (li.getProperty('data-value') == value) {
					var intendedShoeSizeDiv = this.element.getProperty('packageshoesize');
					if(intendedShoeSizeDiv){
						var intendedShoeSizeDivContainer = $(intendedShoeSizeDiv);
						console.log(intendedShoeSizeDivContainer);
						intendedShoeSizeDivContainer.getElements('select').each(function(elmt){
							elmt.hide();
					        elmt.setAttribute('disabled', true);
						});
						intendedShoeSizeDivContainer.getElement('select.'+this.selectOptions[value].shoesizetype).show().removeProperty('disabled');
					}
	...
	updateModifier: function(value){
		//updateAttributeSelection(this.element, this.);
		updateProductOrderFormPrice(value);
	},
	...
	_create: function() {
		...
		this.element.getElements('option').each(function(option) {
			...
			if (o.showImages) {
				...
				this.selectOptions[value].modifier = option.getProperty('modifier');
				this.selectOptions[value].shoesizetype = option.getProperty('shoesizetype');
			}
		}.bind(this));
		...
		Object.each(this.selectOptions, function(option, value) {
			...
			if (o.showImages && option.image) li.adopt(new Element('img.image', { 'src': option.image, 'alt': option.alt, 'modifier':option.modifier, 'shoesizetype':option.shoesizetype}));
