// requires RialtoLoadReplaces
var RialtoTableAccordion = (function(){
	...
	return new Class({
		...
		// default options
		options: {
			className: 'rialtoTableAccordion', // the class name
			...
			accordionRowClass: 'RialtoTableAccordion', // visible row class
			accordionHiddenClass: 'rialtoTableAccordionHidden', // accordion hidden rows class
		...
		initialize: function(options){
			console.log('initializing RialtoTable');
		...
		setup: function(table) {
			if(table.hasClass(self.options.accordionExpandedClass)) {
				self.showRows(table.getElements('.'+self.options.accordionRowClass)); // add current class to all rows
			} else {
				table.getElements('.'+self.options.accordionContentClass).setStyle('height', '0px');
		...
		handleClick: function(rialtoEvent) {
			self.clickRow(rialtoEvent.elmt);
		...
		hideRow: function(row, o) {
			if(!o) o = {};
			//o.onComplete = function(){ alert('sdf');hiddenRow.removeClass(self.options.selectedClass); };
			...
			hiddenContent.removeClass('overflow-visible');
		...
		expandRow: function(row, o) {
			...
			var hiddenContent = self.getHiddenContent(hiddenRow); // get hidden content
			if(!o) o = {};
			o.onComplete = function(){ hiddenContent.addClass('overflow-visible'); };
			...
			//$Rialto.getPlugin('RialtoPlugins').setupContainer(hiddenContent);
			...
			var o = { // set ajax options
				...
				onSuccess: function(responseHTML){
					...
					(function(){
						...
						$Rialto.getPlugin('RialtoPlugins').setupContainer(hiddenRow);
					}).delay($Rialto.timing); // fade out the current content
				},
				onComplete: function(){
					$Rialto.fx.fadeIn(hiddenContent); // fade out the current content
